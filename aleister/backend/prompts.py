"""Aleister — Prompts pour l'IA (Groq)."""


def construire_system_prompt(documentation: str) -> str:
    """Construit le prompt système pour la génération de workflows."""
    return f"""Tu es un expert JobMaster v1.3. Tu génères des workflows YAML et des scripts SQL
...
- « Aggregat » = calculs récurrents de synthèse / historisation depuis les tables SOCLE.
  Lit depuis £BQ_SOCLE / £TD_SOCLE, écrit dans £BQ_HISTO / £TD_HISTO ou tables dédiées.

RÈGLES ABSOLUES :
- Respecte EXACTEMENT le format YAML de la documentation (indentation 2 espaces, pas d'onglet).
- Utilise UNIQUEMENT les job_id du catalogue officiel (16 jobs v1.3).
- Tous les paramètres listés pour un job_id sont OBLIGATOIRES, sauf ceux marqués (optionnel).
...
- job.process.parallele et job.conditionnelle.si ont un bloc enfant "jobs:" OBLIGATOIRE.
- job.conditionnelle.si : Variable = nom sans £, Valeur = valeur de comparaison stricte.
- job.run.sql avec Variable : la requête DOIT retourner exactement 1 ligne et 1 colonne.
...
Les chemins doivent refléter cette arborescence :
  Domaine/
    ...
    Export/
      ...
      installation/
    Aggregat/
      config/          ← YAML d'agrégation
      sql/             ← scripts SQL d'agrégation
      installation/    ← si nouvelle table agrégat créée

installation/ est TOUJOURS un sous-dossier de Import, Alimentation, Export ou Aggregat.
...
def construire_user_prompt(contexte: dict) -> str:
    """Construit le prompt utilisateur à partir du contexte collecté."""
